...
async def send_gemini_request(payload: dict, session: aiohttp.ClientSession, is_streaming: bool = False) -> Response:
    ...
    # Note: These are now async and use the shared session
    proj_id = await get_user_project_id(creds, session)
    if not proj_id:
        return Response(content="Failed to get user project ID.", status_code=500)

    await onboard_user(creds, proj_id, session)
...
def _handle_streaming_response(session: aiohttp.ClientSession, url: str, data: str, headers: dict) -> StreamingResponse:
...
async def _handle_non_streaming_response(resp: aiohttp.ClientResponse) -> Response:
